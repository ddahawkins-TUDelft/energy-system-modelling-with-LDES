...
def figure_compare_muliple_SoCs(df_reference, list_of_test_dfs, label_graph_1, list_of_test_labels):
    ...
    plt.figure(figsize=(10, 5))
    # Plotting 'soc_proxy' vs 'timesteps' for reference data
    plt.plot(df_reference['timesteps'], df_reference['soc_proxy'], label=label_graph_1,color='black')     
    
    for x in range(len(list_of_test_dfs)):
        plt.plot(list_of_test_dfs[x]['timesteps'], list_of_test_dfs[x]['soc_proxy'], label=list_of_test_labels[x], color=colours[x % len(colours)])
