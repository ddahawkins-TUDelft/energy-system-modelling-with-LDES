...
def visualise(
    ...
    #if variables based on error are requested, first identify the reference, and compute its vars first.
    ...
    for model_dict in list_model_dict:
        ...
        if colour_field == 'MAGMe':
            colour_val = get_MACMe(model_dict)
...
def get_MACMe(model_dict):

    if isinstance(model_dict['model'], tsa_model):

        model = model_dict['model'].calliope_model.model
        list_power_caps = model_dict['model'].soc_proxy.params['capacity_weights'].keys()
        list_energy_caps = ['h2_salt_cavern']

    elif isinstance(model_dict['model'], calliope.Model):

        model = model_dict['model']
        list_power_caps = model_dict['params']['list_power_caps']
        list_energy_caps = ['h2_salt_cavern']

    else:
        raise Exception ('Not a valid model type')

    power_capacities, energy_capacities = get_capacities(model, list_power_caps, list_energy_caps)

    return 0

    

def get_capacities(model: calliope.Model, list_power_caps: list, list_energy_caps: list):
    ...
    power_capacities = power_capacities[power_capacities['techs'].isin(list_power_caps)]
    power_capacities = power_capacities[power_capacities['carriers']=='power']
    power_capacities = power_capacities.set_index('techs')
    power_capacities.drop(['nodes', 'carriers'], axis=1,inplace=True)
    ...
    energy_capacities = energy_capacities[energy_capacities['techs'].isin(list_energy_caps)]
    energy_capacities = energy_capacities.set_index('techs')
    energy_capacities.drop(['nodes'], axis=1,inplace=True)


    return power_capacities, energy_capacities