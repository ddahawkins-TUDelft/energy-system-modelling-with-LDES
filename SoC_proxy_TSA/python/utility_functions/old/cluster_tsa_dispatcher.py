...
for name, config in batch_config.items():
    ...
    list_model_dict.append({
        ...
        'name': f'id={m.id[:4]}... {name}', # proxy_wt={tsa_params['matrix_weights']['proxy']} {'with proxy' if use_proxy else 'without proxy'}, k={m.tsa.params['k_periods']}, agg={cluster_method}, rep={rep_method}'
