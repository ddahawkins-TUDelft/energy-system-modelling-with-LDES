...
def compute_costs_correction(df, installed_caps_nominal, dictionary_costs, renewables_fields_and_weights):

    #derive model of costs for LDES technologies
    storage_capex = np.max(df['soc_proxy_LDES'])*dictionary_costs['storage']['capex'] #capex of ldes e.g. salt cavern
    charging_capex = np.max(df['surplus_LDES'])*dictionary_costs['charging']['capex'] #capex of charging infra e.g. electrolyser
    discharging_capex = np.abs(np.min(df['surplus_LDES']))*dictionary_costs['discharging']['capex'] #capex of discharging infra e.g. fuel cell

    storage_opex = np.sum(np.abs(df['surplus_LDES']))*dictionary_costs['storage']['opex'] #capex of ldes e.g. salt cavern
    charging_opex = np.sum(df[df['surplus_LDES']>0]['surplus_LDES'])*dictionary_costs['charging']['opex'] #capex of charging infra e.g. electrolyser
    discharging_opex = -1*np.sum(df[df['surplus_LDES']<0]['surplus_LDES'])*dictionary_costs['discharging']['opex'] #capex of discharging infra e.g. fuel cell

    #calculate the totex for building our storage infrastructure that meets the renewables defecit 
    storage_totex = storage_capex+storage_opex 
    charging_totex = charging_capex+charging_opex
    discharging_totex = discharging_capex+discharging_opex
    

    #derive model of cost rates for renewable technologies
    ...
    weighted_renewable_opex_rate = sum( 
        weights[i] * dictionary_costs[tech]['opex']
        for i, tech in enumerate(renewable_fields)
    ) 

    weighted_renewable_capex_rate = sum( 
        weights[i] * dictionary_costs[tech]['capex']
        for i, tech in enumerate(renewable_fields)
    ) 

    #As we increase renewables, we need less storage overall, and potentially less charging/discharging capacity.


    #isolate the events where renewables are at a deficit and for which the storage is trying to mitigate
    df_discharging_events = df[df['surplus_LDES']<0]
    #compute their mean output during this period, we expect it to be low given that these are renewable defecit events
    mean_cf_during_discharge = np.mean(df_discharging_events['mean_capacity_factor'])
    #calculate the additional nominal capacity to meet this need
    additional_nominal_renewable_capacity = -1*np.mean(df_discharging_events['surplus_LDES'])/mean_cf_during_discharge
    
    #compute the totex for the additional renewables necessary to meet the deficit
    additional_renewable_totex = -1*np.sum(df_discharging_events['surplus_LDES'])*weighted_renewable_opex_rate+additional_nominal_renewable_capacity*weighted_renewable_capex_rate
    # marginal_additional_renewable_totex = additional_renewable_totex/np.sum(np.abs(df['surplus_LDES']))

    correction_factor = (additional_renewable_totex+storage_totex)/(2*storage_totex)
...
def generate_soc_proxy(
    ...
    if dictionary_costs:
        #compute the cost-curtailment correction factor
        df['soc_proxy_LDES'] = df['soc_proxy_LDES']*compute_costs_correction(df,installed_caps_nominal,dictionary_costs,renewables_fields_and_weights)
