...
def run():
    ...
    result.to_csv('simple_weather-year_ldes-model/export/data_cluster_box_plots.csv')
...
def graphic_box_plot_cluster_errors(result):
    ...
    result.drop_duplicates(subset=['Run'], inplace=True)
    result['Cost Error'] = -1* result['Cost Error']
    result['LDES Capacity Error'] = -1* result['LDES Capacity Error']
    ...
    df_cost_design_unpivoted = result[['Run',str_cost,str_abs_design,ldes_err]]
...
# function to plot scatter of errors
def graphic_scatter_cluster_errors(result):
    ...
    # # str_design = 'Mean Capacity Mix Error'
    # str_compound_err = 'Abs Compound Error'
    # df_cost_design_unpivoted = result[['Run',str_cost,str_abs_design,str_compound_err]]
    # df_cost_design_unpivoted=pd.melt(df_cost_design_unpivoted, id_vars=['Run'], var_name='Parameter', value_name='Error')  

    result = result.rename(columns={'Mean Abs Capacity Mix Error': 'Mean Absolute Capacity Mix Error', 'Cost Error': 'Annualised Cost Error'})
    result['Annualised Cost Error'] = -1* result['Annualised Cost Error']
    ...
    WY1_colour = 'rgb(255, 0, 0)'
    WY2_colour = 'rgb(0, 255, 0)'
    WY3_colour = 'rgb(0, 0, 255)'

    fig = go.Figure()

    fig.add_trace(go.Scatter(
        name='Three Year Clusters',
        y=result_n_3['Annualised Cost Error'],
        x=result_n_3['Mean Absolute Capacity Mix Error'],
        text=result_n_3['Run'],
        mode='markers',
        marker=dict(
            size=20,
    ...
    fig.add_trace(go.Scatter(
        name='Two Year Clusters',
        y=result_n_2['Annualised Cost Error'],
        x=result_n_2['Mean Absolute Capacity Mix Error'],
        text=result_n_2['Run'],
        mode='markers',
        marker=dict(
            size=20,
    ...
    fig.add_trace(go.Scatter(
        name='Single Weather Year',
        y=result_n_1['Annualised Cost Error'],
        x=result_n_1['Mean Absolute Capacity Mix Error'],
        text=result_n_1['Run'],
        mode='markers',
        marker=dict(
            size=20,
    ...
    fig.update_layout(
        plot_bgcolor='rgba(255, 255, 255, 0)',
        title=dict(text=f"Annualised Cost Error versus Mean Abs. Capacity Mix Error for different models runs"),
        yaxis = dict(
            title = dict(
                text='Annualised Cost Error',
                font=dict(size=16)
            ),
            zerolinecolor = 'rgba(0, 0, 0, 0.4)',
        ),
    ...
    fig.write_html('simple_weather-year_ldes-model/export/scatter_cluster_error.html', auto_open=True)

result = pd.read_csv('simple_weather-year_ldes-model/export/data_cluster_box_plots.csv')
...
# print(identify_design_candidates(result,0.1,.1))
# graphic_scatter_cluster_errors(result)
